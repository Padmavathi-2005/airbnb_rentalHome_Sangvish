  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response?.status) {
        navigate('/login', { 
          state: { 
            successMessage: response.message || 'Registration successful! Please check your email.' 
          } 
        });
      }
    } catch (err) {
          ...
          <h2 className="text-3xl text-center  font-semibold">Sign Up</h2>
            <div className="md:min-h-screen flex items-center px-4">
              <div className="w-[100%]">
                ...
                type="submit"
                disabled={loading}
                className={`w-full rounded-md bg-pink-500 px-4 py-2 text-white text-sm font-medium hover:bg-pink-600 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 ${
